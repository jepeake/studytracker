...
import { ChevronLeft, ChevronRight, Clock, Calendar, BarChart2, Moon, Sun, Plus, X, PieChartIcon, BarChartIcon } from 'lucide-react'
...
const githubBlues = [
  '#0366d6', '#2188ff', '#79b8ff', '#c8e1ff', '#0a5ae1', '#044289', '#032f62', '#05264c',
]
...
export function FlowAppComponent() {
  ...
  const getChartData = useCallback(() => {
    ...
    return daysInMonth.map(day => {
      ...
      const durations = workTypes.reduce((acc, workType) => {
        acc[workType] = sessionsForDay
      ...
      return {
        date: format(day, 'dd'),
        ...durations,
        total: Object.values(durations).reduce((sum, duration) => sum + duration, 0)
      }
    })
                    ...
                    <div className="flex flex-col items-center gap-4 w-full">
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className="w-full justify-between bg-white dark:bg-[#21262d] border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-[#30363d]">
              ...
              <Card className="bg-white dark:bg-[#161b22] border border-gray-200 dark:border-gray-700 rounded-md shadow-sm">
                ...
                <CardContent className="p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <Button onClick={handlePrevMonth} variant="outline" className="p-2 bg-white dark:bg-[#21262d] border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#30363d]" disabled={selectedMonth === format(new Date(currentDate.getFullYear(), 0), 'yyyy-MM')}>
                    ...
                    <Button onClick={handleNextMonth} variant="outline" className="p-2 bg-white dark:bg-[#21262d] border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#30363d]" disabled={selectedMonth === currentMonth}>
                      <ChevronRight className="h-4 w-4" />
                    </Button>
                  </div>
                  <div className="grid grid-cols-7 gap-2">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                      <div key={day} className="text-center font-medium text-gray-500 dark:text-gray-400">{day}</div>
                    ))}
                    {chartData.map((day, index) => {
                      const [year, month] = selectedMonth.split('-')
                      const currentDay = new Date(parseInt(year), parseInt(month) - 1, parseInt(day.date))
                  ...
                  <div className="mt-4 text-center">
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Total: {chartData.reduce((sum, day) => sum + day.total, 0).toFixed(2)} hours
                    </p>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartData} margin={{ top: 20, right: 30, left: 30, bottom: 5 }}>
                        <XAxis
                          ...
                          textAnchor="end"
                          height={50}
                          tick={{ fontSize: 12, fill: isDarkMode ? '#8b949e' : '#57606a' }}
                        />
                        <YAxis 
                          tick={{ fill: isDarkMode ? '#8b949e' : '#57606a' }}
                          label={{ value: 'Hours', angle: -90, position: 'insideLeft', fill: isDarkMode ? '#8b949e' : '#57606a', dx: -10 }}
                        />
                        <Tooltip
                          contentStyle={{
